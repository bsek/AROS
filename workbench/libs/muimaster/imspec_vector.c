...
/* Improved arrow drawing constants */
#define MIN_ARROW_SIZE 3
#define ARROW_PADDING 1
...
#define AREA_VECTORS 8 /* plenty for triangles */
#define AREA_BUF_WORDS (AREA_VECTORS * 5)
#define MIN_ARROW_SIZE 3
#define MAX_ARROW_SIZE 50

static struct AreaInfo s_area;
static WORD s_areaBuf[AREA_BUF_WORDS];
static struct TmpRas s_tmpRas;
static PLANEPTR s_tmpPlane = NULL;
static ULONG s_tmpSize = 0; /* bytes */
static ULONG s_bw = 0, s_bh = 0;
static BOOL s_ready = FALSE;

/* Initialise once with the RastPort you'll be drawing into */
BOOL init_arrow_drawer(struct RastPort *rp) {
  if (!rp || !rp->BitMap)
    return FALSE;

  /* Get the full bitmap size, not just arrow size */
  s_bw = GetBitMapAttr(rp->BitMap, BMA_WIDTH);
  s_bh = GetBitMapAttr(rp->BitMap, BMA_HEIGHT);

  s_tmpPlane = AllocRaster(s_bw, s_bh);
  if (!s_tmpPlane)
    return FALSE;

  s_tmpSize = RASSIZE(s_bw, s_bh);
  InitTmpRas(&s_tmpRas, s_tmpPlane, s_tmpSize);

  InitArea(&s_area, s_areaBuf, AREA_VECTORS);
...
void cleanup_arrow_drawer(void) {
  if (s_tmpPlane) {
    FreeRaster(s_tmpPlane, s_bw, s_bh);
    s_tmpPlane = NULL;
  }
  s_ready = FALSE;
}

/* Draw a solid filled arrow */
void draw_filled_triangle_arrow(struct RastPort *rp, LONG left, LONG top,
                                LONG width, LONG height, ULONG direction) {
  init_arrow_drawer(rp);
  ...
  LONG arrow_size = MIN(width, height) / 3;

  if (arrow_size < MIN_ARROW_SIZE)
    arrow_size = MIN_ARROW_SIZE;
  if (arrow_size > MAX_ARROW_SIZE)
    arrow_size = MAX_ARROW_SIZE;

  /* Save old AreaInfo/TmpRas */
  struct AreaInfo *oldArea = rp->AreaInfo;
  struct TmpRas *oldTmp = rp->TmpRas;

  /* Use persistent ones */
  rp->AreaInfo = &s_area;
  rp->TmpRas = &s_tmpRas;

  /* Ensure solid fill and known mode */
  // SetAfPt(rp, NULL, 0); /* Disable patterns */
  SetDrMd(rp, JAM1);
  /* SetAPen(rp, 1); // set your desired pen colour here */

  /* Clear scratch TmpRas to avoid leftovers */
  BltClear(s_tmpPlane, s_tmpSize, 1);

  switch (direction) {
  case 0: /* UP */
    AreaMove(rp, cx, cy - arrow_size);
    AreaDraw(rp, cx - arrow_size, cy + arrow_size);
    AreaDraw(rp, cx + arrow_size, cy + arrow_size);
    AreaEnd(rp);
    break;
  ...
  /* Restore caller's original state */
  rp->AreaInfo = oldArea;
  rp->TmpRas = oldTmp;

  cleanup_arrow_drawer();
}

/*
 * Fallback line-based arrow drawing with improved algorithm
 * Cleaner and more consistent than the original implementation
 */
static void draw_line_triangle_arrow(struct RastPort *rp, LONG left, LONG top,
                                     LONG width, LONG height, ULONG direction) {
  LONG cx = left + width / 2;
  LONG cy = top + height / 2;
  LONG arrow_size = MIN(width, height) / 3;
  ...
  case 0: /* ARROW_UP */
    ...
    Move(rp, cx, cy - arrow_size);
    Draw(rp, cx - arrow_size, cy + arrow_size);
    Draw(rp, cx + arrow_size, cy + arrow_size);
    Draw(rp, cx, cy - arrow_size);

    /* Add internal lines for better visibility at larger sizes */
    if (arrow_size >= 4) {
      Move(rp, cx - arrow_size / 2, cy + arrow_size / 2);
      Draw(rp, cx + arrow_size / 2, cy + arrow_size / 2);
    }
    break;
  ...
  case 2: /* ARROW_LEFT */
    Move(rp, cx - arrow_size, cy);
    Draw(rp, cx + arrow_size, cy - arrow_size);
    Draw(rp, cx + arrow_size, cy + arrow_size);
    Draw(rp, cx - arrow_size, cy);
...
/*
 * Alternative chevron-style arrows for a more modern look
 * These use angled lines instead of triangles
 */

static void draw_chevron_arrow(struct RastPort *rp, LONG left, LONG top,
                               LONG width, LONG height, ULONG direction) {
  LONG cx = left + width / 2;
  LONG cy = top + height / 2;
  LONG chevron_size = MIN(width, height) / 4;
  ...
  switch (direction) {
  case 0: /* CHEVRON_UP */
    Move(rp, cx - chevron_size, cy + chevron_size / 2);
    Draw(rp, cx, cy - chevron_size / 2);
    Draw(rp, cx + chevron_size, cy + chevron_size / 2);
    break;
  ...
  case 2: /* CHEVRON_LEFT */
    Move(rp, cx + chevron_size / 2, cy - chevron_size);
    Draw(rp, cx - chevron_size / 2, cy);
    Draw(rp, cx + chevron_size / 2, cy + chevron_size);
    break;

  case 3: /* CHEVRON_RIGHT */
    Move(rp, cx - chevron_size / 2, cy - chevron_size);
    Draw(rp, cx + chevron_size / 2, cy);
    Draw(rp, cx - chevron_size / 2, cy + chevron_size);
    break;
...
/* Chevron-style arrow functions for alternative UI style */
void chevron_arrowup_draw(struct MUI_RenderInfo *mri, LONG left, LONG top,
  ...
  draw_chevron_arrow(rp, left, top, width, height, 0);
}
...
void chevron_arrowdown_draw(struct MUI_RenderInfo *mri, LONG left, LONG top,
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_chevron_arrow(rp, left, top, width, height, 1);
}
...
void chevron_arrowleft_draw(struct MUI_RenderInfo *mri, LONG left, LONG top,
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_chevron_arrow(rp, left, top, width, height, 2);
}
...
void chevron_arrowright_draw(struct MUI_RenderInfo *mri, LONG left, LONG top,
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_chevron_arrow(rp, left, top, width, height, 3);
}

void arrowup_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_filled_triangle_arrow(rp, left, top, width, height, 0);
}

void arrowdown_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
                    LONG height, LONG state) {
  struct RastPort *rp = mri->mri_RastPort;
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_filled_triangle_arrow(rp, left, top, width, height, 1);
}

void arrowleft_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
                    LONG height, LONG state) {
  struct RastPort *rp = mri->mri_RastPort;
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_filled_triangle_arrow(rp, left, top, width, height, 2);
}
...
void arrowright_draw(struct MUI_RenderInfo *mri, LONG left, LONG top,
  ...
  SetAPen(rp, mri->mri_Pens[MPEN_TEXT]);
  draw_filled_triangle_arrow(rp, left, top, width, height, 3);
}

void checkbox_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
                   LONG height, LONG state) {
...
void tape_play_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
  ...
  for (x = 0; x < width; x++) {
    ...
    RectFill(rport, left + width - 1 - x, top + half - y, left + width - 1 - x,
             top + half + y);
  }
}
...
void tape_down_draw(struct MUI_RenderInfo *mri, LONG left, LONG top, LONG width,
  ...
  for (y = 0; y < height; y++) {
    ...
    RectFill(rport, left + half - x, top + height - 1 - y, left + half + x,
             top + height - 1 - y);
  }
}
...
static const struct vector_image vector_table[] = {
    ...
    /* Chevron-style arrows for modern UI themes */
    {8 + 2 * ARROW_SPACING, 8 + 2 * ARROW_SPACING, chevron_arrowup_draw},
    {8 + 2 * ARROW_SPACING, 8 + 2 * ARROW_SPACING, chevron_arrowdown_draw},
    {8 + 2 * ARROW_SPACING, 8 + 2 * ARROW_SPACING, chevron_arrowleft_draw},
    {8 + 2 * ARROW_SPACING, 8 + 2 * ARROW_SPACING, chevron_arrowright_draw},
};
