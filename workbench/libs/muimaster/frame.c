...
static void frame_ultra_rounded_draw(struct dt_frame_image *fi,
  ...
  /* Clear corner areas with background */
  SetAPen(rp, pens.background);

  /* Top-left corner area */
  RectFill(rp, left, top, left + radius, top + radius);
  /* Top-right corner area */
  RectFill(rp, left + width - radius - 1, top, left + width - 1, top + radius);
  /* Bottom-left corner area */
  RectFill(rp, left, top + height - radius - 1, left + radius,
           top + height - 1);
  /* Bottom-right corner area */
  RectFill(rp, left + width - radius - 1, top + height - radius - 1,
...
static void frame_enhanced_rounded_draw(struct dt_frame_image *fi,
  ...
  /* Clear corner regions with background - exact corner squares */
  SetAPen(rp, pens.background);
  RectFill(rp, left, top, left + radius, top + radius);
  RectFill(rp, left + width - radius - 1, top, left + width - 1, top + radius);
  RectFill(rp, left, top + height - radius - 1, left + radius,
