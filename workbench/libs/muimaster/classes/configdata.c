...
const static struct spec_cfg DefFramespecValues[] = {
    ...
    {MUIV_Frame_ImageButton, MUICFG_Frame_ImageButton,
     "202211"}, /* image button             */
    {MUIV_Frame_Text, MUICFG_Frame_Text,
     "212211"}, /* textfield without input  */
    {MUIV_Frame_String, MUICFG_Frame_String, "302211"}, /* string gadget */
    {MUIV_Frame_ReadList, MUICFG_Frame_ReadList,
     "212211"}, /* list without input       */
    {MUIV_Frame_InputList, MUICFG_Frame_InputList,
     "202211"}, /* list with input          */
    ...
    {MUIV_Frame_PopUp, MUICFG_Frame_PopUp,
     "112211"}, /* cycle menu, popup window */
...
static void init_framespecs(Object *obj, struct MUI_ConfigdataData *data) {
  ...
  for (i = 0; DefFramespecValues[i].defspec; i++) {
    ...
    zune_frame_spec_to_intern(framespec, &data->prefs.frames[fcfg->muiv]);
  }
}
...
const static struct def_ulval DefULValues[] = {
    ...
    {MUICFG_Register_TabHeightExtra, 12},
    {MUICFG_Register_TextSpacing, 8},
    {MUICFG_Register_IntertabSpacing, 6},
    {MUICFG_Register_FrameX, 8},
    {MUICFG_Register_FrameBottom, 6},
    {MUICFG_Register_FrameTop, 5},
...
IPTR Configdata__OM_NEW(struct IClass *cl, Object *obj, struct opSet *msg) {
  ...
  data->prefs.register_tab_height_extra =
      GetConfigULong(obj, MUICFG_Register_TabHeightExtra);
  data->prefs.register_text_spacing =
      GetConfigULong(obj, MUICFG_Register_TextSpacing);
  data->prefs.register_intertab_spacing =
      GetConfigULong(obj, MUICFG_Register_IntertabSpacing);
  data->prefs.register_frame_x = GetConfigULong(obj, MUICFG_Register_FrameX);
  data->prefs.register_frame_bottom =
      GetConfigULong(obj, MUICFG_Register_FrameBottom);
  data->prefs.register_frame_top =
      GetConfigULong(obj, MUICFG_Register_FrameTop);
  ...
  for (i = 0; i < MUIKEY_COUNT; i++) {
    if (data->prefs.muikeys[i].readable_hotkey)
      data->prefs.muikeys[i].ix_well = !ParseIX(
          data->prefs.muikeys[i].readable_hotkey, &data->prefs.muikeys[i].ix);
    else
...
IPTR Configdata__MUIM_SetImspec(struct IClass *cl, Object *obj,
  ...
  for (i = 0; DefImspecValues[i].defspec; i++) {
    if (DefImspecValues[i].cfgid == msg->id)
      if (!strcmp(DefImspecValues[i].defspec, msg->imspec)) {
        /*                  D(bug("Configdata_SetImspec(%p) : set to def, id
         * %08lx, val %s\n", */
  ...
  DoMethod(obj, MUIM_Dataspace_Add, (IPTR)msg->imspec, strlen(msg->imspec) + 1,
           msg->id);
  return 0;
}
...
IPTR Configdata__MUIM_SetFont(struct IClass *cl, Object *obj,
  ...
  DoMethod(obj, MUIM_Dataspace_Add, (IPTR)msg->font, strlen(msg->font) + 1,
           msg->id);
  return 0;
}
...
IPTR Configdata__MUIM_SetString(struct IClass *cl, Object *obj,
  ...
  DoMethod(obj, MUIM_Dataspace_Add, (IPTR)msg->string, strlen(msg->string) + 1,
           msg->id);
  return 0;
}
      ...
      if (!OpenIFF(iff, IFFF_WRITE)) {
        if (!PushChunk(iff, MAKE_ID('P', 'R', 'E', 'F'), ID_FORM,
          ...
          if (SavePrefsHeader(iff)) {
            DoMethod(obj, MUIM_Dataspace_WriteIFF, (IPTR)iff, 0,
                     MAKE_ID('M', 'U', 'I', 'C'));
          }
          PopChunk(iff);
        }
