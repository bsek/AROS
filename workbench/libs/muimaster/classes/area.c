...
static IPTR Area__OM_NEW(struct IClass *cl, Object *obj, struct opSet *msg) {
  ...
  data->mad_FrameClipHandle = (APTR)-1;
  ...
  for (tags = msg->ops_AttrList; (tag = NextTagItem(&tags));) {
    ...
    case MUIA_FrameClippingHook:
      data->mad_FrameClippingHook = (struct Hook *)tag->ti_Data;
      break;
    }
  }
...
#define STORE *(msg->opg_Storage)
  ...
  case MUIA_FrameClippingHook:
    STORE = (IPTR)data->mad_FrameClippingHook;
    return TRUE;
...
static IPTR Area__MUIM_Draw(struct IClass *cl, Object *obj,
  ...
  /*      D(bug(" Area_Draw(%p) msg=0x08lx flags=0x%08lx\n", obj, msg->flags,
              _flags(obj))); */

  /* Clean up any previous frame clipping from this object before starting new
   * draw */
  if (data->mad_FrameClipHandle != (APTR)-1) {
    D(bug("DEBUG: Removing previous frame clip handle %p for object %p\n",
          data->mad_FrameClipHandle, obj));
    MUI_RemoveClipRegion(muiRenderInfo(obj), data->mad_FrameClipHandle);
    data->mad_FrameClipHandle = (APTR)-1;
    D(bug("DEBUG: Previous frame clipping removed\n"));
  }
   ...
   */
  {
    ...
    /* Call frame clipping hook if present to set up clipping for rounded
     * corners */
    APTR frame_clip = (APTR)-1;
    if (data->mad_FrameClippingHook) {
      D(bug("AREA: Found FrameClippingHook for object %p\n", obj));
      struct MUI_FrameClipInfo clipinfo;
      if (zune_frame_get_clip_info(obj, frame, &clipinfo)) {
        D(bug("AREA: Got clip info - radius=%d, has_rounded=%d\n",
              clipinfo.border_radius, clipinfo.has_rounded_corners));
        struct Region *clipregion = NULL;
        struct MUIP_FrameClippingHook hook_msg;
        hook_msg.MethodID = MUIM_QueryFrameClipping;
        hook_msg.obj = obj;
        hook_msg.clipinfo = &clipinfo;
        hook_msg.clipregion = &clipregion;

        CallHookPkt(data->mad_FrameClippingHook, obj, &hook_msg);

        D(bug("AREA: Hook returned clipregion=%p\n", clipregion));
        if (clipregion) {
          D(bug("AREA: Installing clip region - bounds (%d,%d)-(%d,%d)\n",
                clipregion->bounds.MinX, clipregion->bounds.MinY,
                clipregion->bounds.MaxX, clipregion->bounds.MaxY));
        }

        /* Install clipping region if hook provided one */
        if (clipregion) {
          struct MUI_RenderInfo *mri = muiRenderInfo(obj);
          D(bug("DEBUG: Installing frame clip region %p for object %p\n",
                clipregion, obj));
          frame_clip = MUI_AddClipRegion(mri, clipregion);
          data->mad_FrameClipHandle = frame_clip;
          D(bug("DEBUG: MUI_AddClipRegion returned handle %p\n", frame_clip));
          if (frame_clip == (APTR)-1) {
            D(bug("DEBUG: MUI_AddClipRegion FAILED - returned -1\n"));
          } else {
            D(bug("DEBUG: MUI_AddClipRegion SUCCESS - old region was %p\n",
                  frame_clip));
          }
          D(bug("AREA: MUI_AddClipRegion returned %p (old region)\n",
                frame_clip));
        }
      } else {
        D(bug("AREA: Failed to get clip info for object %p\n", obj));
      }
    } else {
      D(bug("AREA: No FrameClippingHook for object %p\n", obj));
    }
    ...
    /* Frame clipping is now active and will be removed at the end of this
     * method This ensures child objects can draw their content with clipping
     * active */
  }

  /*    MUI_RemoveClipping(muiRenderInfo(obj), areaclip);*/

  /* Frame clipping stays active for child class drawing and will be removed
   * when this object draws again, is hidden, or cleaned up */
...
Area__MUIM_DrawBackgroundBuffered(struct IClass *cl, Object *obj,
  ...
  /*      D(bug("Area_DrawBackground(%p): draw bg\n", obj)); */
  zune_imspec_drawbuffered(bg, _rp(obj), data->mad_RenderInfo, msg->left,
...
static IPTR Area__MUIM_Cleanup(struct IClass *cl, Object *obj,
  ...
  /* Clean up frame clipping during cleanup */
  if (data->mad_FrameClipHandle != (APTR)-1) {
    D(bug("DEBUG: Cleanup - removing frame clip handle %p for object %p\n",
          data->mad_FrameClipHandle, obj));
    MUI_RemoveClipRegion(muiRenderInfo(obj), data->mad_FrameClipHandle);
    data->mad_FrameClipHandle = (APTR)-1;
    D(bug("DEBUG: Cleanup frame clipping removed\n"));
  }
...
static IPTR Area__MUIM_Hide(struct IClass *cl, Object *obj,
  ...
  /* Clean up frame clipping when hiding */
  if (data->mad_FrameClipHandle != (APTR)-1) {
    D(bug("DEBUG: Hide - removing frame clip handle %p for object %p\n",
          data->mad_FrameClipHandle, obj));
    MUI_RemoveClipRegion(muiRenderInfo(obj), data->mad_FrameClipHandle);
    data->mad_FrameClipHandle = (APTR)-1;
    D(bug("DEBUG: Hide frame clipping removed\n"));
  }
...
**************************************************************************/
static IPTR Area__MUIM_QueryFrameClipping(struct IClass *cl, Object *obj,
                                          struct MUIP_QueryFrameClipping *msg) {
  struct MUI_AreaData *data = INST_DATA(cl, obj);
  const struct MUI_FrameSpec_intern *frame;
  struct MUI_FrameSpec_intern tempframe;
  ...
  frame = get_intframe(obj, data, &tempframe);
  return zune_frame_get_clip_info(obj, frame, msg->clipinfo);
}
...
BOOPSI_DISPATCHER(IPTR, Area_Dispatcher, cl, obj, msg) {
  ...
  case MUIM_QueryFrameClipping:
    return Area__MUIM_QueryFrameClipping(cl, obj, (APTR)msg);
  }
