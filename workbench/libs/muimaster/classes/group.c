...
#define IS_HIDDEN(obj)                                                         \
  (!(_flags(obj) & MADF_SHOWME) || (_flags(obj) & MADF_BORDERGADGET))
...
static IPTR Group__MUIM_AddObject(struct IClass *cl, Object *obj, Msg msg) {
  ...
  struct TagItem nestedDisableAttrs[4] = {{MUIA_NoNotify, TRUE},
...
IPTR Group__OM_SET(struct IClass *cl, Object *obj, struct opSet *msg) {
  ...
  if (forward) {
    ...
    struct TagItem nestedAttrs[2] = {{MUIA_NestedDisabled, FALSE},
  ...
  if (disableTag) {
    struct TagItem disableAttrs[2] = {{MUIA_Disabled, disableTag->ti_Data},
                                      {TAG_DONE, 0}};
...
IPTR Group__MUIM_Draw(struct IClass *cl, Object *obj, struct MUIP_Draw *msg) {
  ...
  if ((msg->flags & MADF_DRAWUPDATE) && data->update == 1) {
     ...
     */
    DoMethod(obj, MUIM_DrawBackground, _mleft(obj), _mtop(obj), _mwidth(obj),
             _mheight(obj), _mleft(obj), _mtop(obj), 0);
    ...
    if ((msg->flags & MADF_DRAWUPDATE) && data->update == 2) {
      ...
      /* sba: I don't know how MUI handle this but ScrollRasterBF() made
       *problems when scrolling
       ** a (partly visible) virtual groups in a virtual group, because e.g.
       *_mtop() is then
       ** smaller than the region. ScrollRasterBF() on AmigaOS then marks the
       *complete region
       ** as damaged. Using ScrollWindowRaster() solved that problem but it
       *flickers then.
       ** To avoid this we prevent that the scroll area is out of the region
       *bounds.
       ** The region bounds are setted in MUI_Redraw() but should probably
       *should go in the
      ...
      /* old code was
       ** ScrollRasterBF(_rp(obj), diff_virt_offx, diff_virt_offy, _mleft(obj),
       *_mtop(obj), _mright(obj),_mbottom(obj));
       */

      ScrollWindowRaster(_window(obj), diff_virt_offx, diff_virt_offy, left,
                         top, right, bottom);
  ...
  while ((child = NextObject(&cstate))) {
    ...
    if ((data->flags & GROUP_PAGEMODE) &&
        ((page != data->active_page) && (child != data->titlegroup))) {
      continue;
    }
  ...
  /* Remove any frame clipping that may be active from child objects
   * This ensures clipping doesn't affect objects outside this group */
  {
    struct MUI_RenderInfo *mri = muiRenderInfo(obj);

    if (mri && mri->mri_rCount > 0) {
      D(bug("GROUP: Found %d active clip regions after children drawn, "
            "removing all for object %p\n",
            mri->mri_rCount, obj));
      ...
      while ((child = NextObject(&cstate))) {
        if (!(_flags(child) & MADF_SHOWME))
          continue;
        if (muiAreaData(child) != NULL) {
          D(bug("GROUP: Found area data for object %p\n", child));
          APTR frameClipHandle = muiAreaData(child)->mad_FrameClipHandle;
          D(bug("GROUP: FrameClipHandle: %p\n"), frameClipHandle);
          if (frameClipHandle != (APTR)-1) {
            D(bug("GROUP: Found active frame clip region after children drawn, "
                  "removing for child %p\n",
                  child));
            MUI_RemoveClipRegion(mri, frameClipHandle);
            muiAreaData(child)->mad_FrameClipHandle = (APTR)-1;
          }
        }
      }
    }
...
static void minmax_2d_rows_pass(struct MUI_GroupData *data,
                                struct MinList *children,
                                struct MUI_MinMax *req, WORD maxmin_height,
                                WORD maxdef_height) {
  int i, j;
  APTR cstate;
  Object *child;
...
static void minmax_2d_columns_pass(struct MUI_GroupData *data,
                                   struct MinList *children,
                                   struct MUI_MinMax *req, WORD maxmin_width,
                                   WORD maxdef_width) {
  int i, j;
  APTR cstate;
  Object *child;
...
static void group_minmax_2d(struct IClass *cl, Object *obj,
                            struct MinList *children,
                            struct MUIP_AskMinMax *msg) {
  struct MUI_GroupData *data = INST_DATA(cl, obj);
  APTR cstate;
...
static void group_minmax_pagemode(struct IClass *cl, Object *obj,
                                  struct MinList *children,
                                  struct MUIP_AskMinMax *msg) {
  APTR cstate;
  Object *child;
  ...
  while ((child = NextObject(&cstate))) {
    ...
    tmp.DefHeight = MAX(tmp.DefHeight, ((w0_defheight(child) < MUI_MAXMAX)
                                            ? w0_defheight(child)
                                            : tmp.DefHeight));
    tmp.DefWidth = MAX(tmp.DefWidth,
                       ((w0_defwidth(child) < MUI_MAXMAX) ? w0_defwidth(child)
                                                          : tmp.DefWidth));
    D(bug("minmax_pagemode(0x%p) defw = %ld\n", obj, tmp.DefWidth));
  }
...
// while we're at it
static void Layout1D_minmax_constraint(WORD *sizep, WORD minsize, WORD maxsize,
                                       WORD *remainp, WORD *sizegrowp,
                                       WORD *sizeshrinkp, ULONG *weightgrowp,
                                       ULONG *weightshrinkp, UWORD weight,
                                       WORD samesize) {
  WORD size = *sizep, remain = *remainp, sizegrow = *sizegrowp,
       sizeshrink = *sizeshrinkp;
...
// child
static void Layout1D_redistribution(WORD *sizep, WORD minsize, WORD maxsize,
                                    WORD *remainp, WORD *sizegrowp,
                                    WORD *sizeshrinkp, ULONG *weightgrowp,
                                    ULONG *weightshrinkp, UWORD weight) {
  WORD size = *sizep, remain = *remainp, sizegrow = *sizegrowp,
...
// with an example)
static void Layout1D_minmax_constraints_and_redistrib(struct MinList *children,
                                                      WORD total_size,
                                                      WORD remainder,
                                                      WORD samesize,
                                                      BOOL group_horiz) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    while ((child = NextObject(&cstate))) {
      ...
      if (group_horiz) {
        ...
        Layout1D_minmax_constraint(
            &_width(child), _minwidth(child), _maxwidth(child), &remainder,
         ...
         */
        /*                    child, _width(child), old_size, remainder,
         * total_size, */
      ...
      } else // ! group_horiz
      {
        ...
        Layout1D_minmax_constraint(
            &_height(child), _minheight(child), _maxheight(child), &remainder,
            &size_growables, &size_shrinkables, &weight_growables,
        ...
        /*                    "sizegrow=%d, sizeshrink=%d w=%d min=%d max=%d\n",
         */
        /*                    child, _height(child), old_size, remainder,
         * total_size,*/
    ...
    while (((child = NextObject(&cstate)) != NULL) && (remainder != 0)) {
      ...
      if (group_horiz) {
        ...
        /*                        _width(child), old_size, remainder,
         * total_size, */
        /*                        size_growables, size_shrinkables)); */
      } else // ! group_horiz
      {
        ...
        /*                  D(bug("loop2 on %p : h=%d was %d, rem=%d, A=%d, " */
        /*                        "size_grow=%d, size_shrink=%d\n", child, */
        /*                        _height(child), old_size, remainder,
         * total_size, */
        /*                        size_growables, size_shrinkables)); */
      } // if (group_horiz)
    ...
    /*          if (remainder != 0) */
    /*          { */
    /*              D(bug("end : rem=%ld, A=%ld, size_grow=%ld,
     * size_shrink=%ld\n", */
    /*                    remainder, total_size, size_growables,
     * size_shrinkables)); */
    /*          } */
...
static void Layout1D_weight_constraint(WORD *total_sizep,
                                       ...
                                       ULONG *total_weightp, WORD *sizep,
                                       UWORD weight, WORD minsize) {
  if (*total_weightp > 0)
    *sizep = (*total_sizep * weight + *total_weightp / 2) / *total_weightp;
  else
    *sizep = minsize;
...
static void group_layout_vert(struct IClass *cl, Object *obj,
  ...
  total_init_size = 0;
  total_size =
      layout_height - (data->num_visible_children - 1) * data->vert_spacing;
  ...
  while ((child = NextObject(&cstate))) {
    ...
    Layout1D_weight_constraint(&total_size, &total_init_size, &total_weight,
                               &_height(child), _vweight(child),
                               _minheight(child));
...
static void group_layout_horiz(struct IClass *cl, Object *obj,
  ...
  total_init_size = 0;
  total_size =
      layout_width - (data->num_visible_children - 1) * data->horiz_spacing;
  total_size_backup = total_size;
  ...
  Layout1D_minmax_constraints_and_redistrib(
      children, total_size, remainder,
      (data->flags & GROUP_SAME_WIDTH) ? data->samesize_maxmin_horiz : 0, TRUE);
...
static void Layout2D_weight_constraint(
    struct MUI_GroupData *data, struct layout2d_elem *row_infos,
    struct layout2d_elem *col_infos, WORD total_size_height,
    WORD total_size_width, WORD *total_init_height, WORD *total_init_width) {
  int i;
  ...
  *total_init_width = 0;
  ...
  for (i = 0; i < data->rows; i++) {
    if (total_weight_vert > 0)
      row_infos[i].dim =
          (total_size_height * row_infos[i].weight + total_weight_vert / 2) /
          total_weight_vert;
    else
  ...
  for (i = 0; i < data->columns; i++) {
    if (total_weight_horiz)
      col_infos[i].dim =
          (total_size_width * col_infos[i].weight + total_weight_horiz / 2) /
          total_weight_horiz;
    else
...
static void
Layout2D_minmax_constraints_and_redistrib(struct layout2d_elem *infos,
                                          WORD nitems, WORD total_size,
                                          WORD samesize, WORD remainder) {
  int j;
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < nitems; i++) {
      ...
      /*                        "sizegrow=%d, sizeshrink=%d w=%d min=%d
       * max=%d\n", */
      ...
      Layout1D_minmax_constraint(&infos[i].dim, infos[i].min, infos[i].max,
                                 &remainder, &size_growables, &size_shrinkables,
      ...
      /*                        "sizegrow=%d, sizeshrink=%d w=%d min=%d
       * max=%d\n", */
      /*                        i, infos[i].dim, old_size, remainder,
       * total_size, */
      /*                        size_growables, size_shrinkables,
       * infos[i].weight, */
      /*                        infos[i].min, infos[i].max)); */
    ...
    for (i = 0; i < nitems; i++) {
      ...
      Layout1D_redistribution(&infos[i].dim, infos[i].min, infos[i].max,
                              &remainder, &size_growables, &size_shrinkables,
                              &weight_growables, &weight_shrinkables,
                              infos[i].weight);
...
static void layout_2d_distribute_space(struct MUI_GroupData *data,
                                       struct layout2d_elem *row_infos,
                                       struct layout2d_elem *col_infos,
                                       struct MinList *children,
...
static void group_layout_2d(struct IClass *cl, Object *obj,
                            struct MinList *children) {
  struct MUI_GroupData *data = INST_DATA(cl, obj);
  WORD left_start = 0;
  WORD top_start = 0;
  ...
  if (data->rows == 0 || data->columns == 0)
    return;
  if (data->num_children % data->rows || data->num_children % data->columns)
    return;
  ...
  Layout2D_minmax_constraints_and_redistrib(
      data->row_infos, data->rows, total_size_height,
      /*      (data->flags & GROUP_SAME_HEIGHT) ? data->samesize_maxmin_vert :
         0, */
      0, remainder_height);

  Layout2D_minmax_constraints_and_redistrib(
      data->col_infos, data->columns, total_size_width,
      /*      (data->flags & GROUP_SAME_WIDTH) ? data->samesize_maxmin_horiz :
         0, */
      0, remainder_width);

  layout_2d_distribute_space(data, data->row_infos, data->col_infos, children,
  ...
  if ((data->flags & GROUP_VIRTUAL) && msg->imsg) {
    ...
    case IDCMP_MOUSEBUTTONS:
      ...
      if (msg->imsg->Code == SELECTDOWN) {
        if (_between(_mleft(obj), msg->imsg->MouseX, _mright(obj)) &&
            _between(_mtop(obj), msg->imsg->MouseY, _mbottom(obj))) {
          DoMethod(_win(obj), MUIM_Window_RemEventHandler, (IPTR)&data->ehn);
    ...
    case IDCMP_INTUITICKS:
      ...
      if (!(_between(_mleft(obj), msg->imsg->MouseX, _mright(obj)) &&
        ...
        if (new_virt_offx != data->virt_offx ||
            new_virt_offy != data->virt_offy) {
          SetAttrs(obj, MUIA_Virtgroup_Left, new_virt_offx, MUIA_Virtgroup_Top,
                   new_virt_offy, MUIA_Group_Forward, FALSE, TAG_DONE);
        }
      }
      break;
    }
 ...
 * Class descriptor.
 */
const struct __MUIBuiltinClass _MUI_Group_desc = {MUIC_Group, MUIC_Area,
