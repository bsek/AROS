/*
    ...
    ZuneGfx Library - Polygon Demo (Unlocked DrawingBoard)

    Tests polygon fill with standard rastport operations:
    - ZuneFillPolygon with solid, gradient, and texture brushes

    Usage: demo_polygon_unlocked [cybergfx|opengl]
*/
...
static void DrawPolygons(struct DemoContext *ctx)
{
    ...
    /* Texture-filled (from testimage.png) */
...
int main(int argc, char **argv)
{
    ...
    printf("Polygon Demo (Unlocked) - %s\n", DemoBackendName(backend));
    printf("==========================================\n\n");

    if (!DemoInit(&ctx, "Polygon Demo (Unlocked)", 800, 480, backend, 0))
        return 1;
