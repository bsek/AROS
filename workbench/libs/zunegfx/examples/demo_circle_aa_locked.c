/*
    ...
    ZuneGfx Library - AA Circle Demo (Locked DrawingBoard)

    Tests antialiased circle functions with locked pixel access:
    - ZuneFillCircleAA
    - ZuneFillCircleStyledAA (fill + border)
    - ZuneDrawCircleOutlineStyledAA

    Usage: demo_circle_aa_locked [cybergfx|opengl]
*/
...
int main(int argc, char **argv)
{
    ...
    printf("AA Circle Demo (Locked) - %s\n", DemoBackendName(backend));
    printf("========================================\n\n");

    if (!DemoInit(&ctx, "AA Circle Demo (Locked)", 640, 480, backend, ZUNE_DRAWINGBOARD_LINEARMEM))
        return 1;
