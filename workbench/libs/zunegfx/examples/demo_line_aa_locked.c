/*
    ...
    ZuneGfx Library - AA Line Demo (Locked DrawingBoard)

    Tests antialiased line functions with locked pixel access:
    - ZuneDrawLineAA
    - ZuneDrawLineStyledAA (with thickness)

    Usage: demo_line_aa_locked [cybergfx|opengl]
*/
...
int main(int argc, char **argv)
{
    ...
    printf("AA Line Demo (Locked) - %s\n", DemoBackendName(backend));
    printf("======================================\n\n");

    if (!DemoInit(&ctx, "AA Line Demo (Locked)", 640, 480, backend, ZUNE_DRAWINGBOARD_LINEARMEM))
        return 1;
