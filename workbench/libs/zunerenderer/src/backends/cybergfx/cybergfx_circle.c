...
static void CybergfxDrawAACircleRastPort(struct RenderPort *rp, UWORD x, UWORD y, UWORD radius, struct ZuneBrush *fill_brush,
    ...
    for (int py = min_y; py <= max_y; py++) {
        BOOL scan_ok = FALSE;
        if (row_buffer) {
            scan_ok = ReadPixelArray(row_buffer, 0, 0, (ULONG)width * 4, rp->target_rp, min_x, py, width, 1, CYBERGFX_PIXELFORMAT_ARGB32);
        }

        float rel_y = (py + 0.5f) - y;
        int px = 0;

        if (scan_ok && cybergfx_get_simd_level() == CYBERGFX_SIMD_AVX2) {
            for (; px <= width - 8; px += 8) {
        ...
        for (; px < width; px++) {
            ...
            ULONG bg = scan_ok ? row_buffer[px] : ReadRGBPixel(rp->target_rp, min_x + px, py);
            UBYTE bg_a, bg_r, bg_g, bg_b;
        ...
        if (scan_ok) {
            WritePixelArray(row_buffer, 0, 0, (ULONG)width * 4, rp->target_rp, min_x, py, width, 1, CYBERGFX_PIXELFORMAT_ARGB32);
        }
    }
