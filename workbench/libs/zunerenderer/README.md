...
# Install includes
mmake workbench-libs-zunerenderer-includes
```

The library will be installed to `LIBS:zunerenderer.library` and headers to `INCLUDE:libraries/zunerenderer.h`.
...
├── include/
│   └── zunerenderer.h          # Public API header
├── src/
│   ├── zunerenderer_intern.h   # Internal definitions
...
For more information, see the example programs in the `examples/` directory and the complete API documentation in `include/zunerenderer.h`.
