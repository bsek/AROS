 ...
 * brcm,msi-offset, which names the first message the block owns - the
 * SPIs it raises start at base + offset, not at base.
 *
 * Each host bridge has its own block, so all of them are consulted: a
 * range belonging to a bridge nothing drives costs only an edge setting
 * on an SPI that never fires.
 */
