 ...
 * the ones SetupInbound() programs.
 */
...
/*
 * Read the bridge's translation state back out of its registers. This runs
 * for a bridge we just trained as well as for one the firmware left running,
 * so everything downstream sees the same thing: what the hardware will
 * actually do, rather than what a property says it ought to.
 *
 * That distinction is not academic. On the x4 link the firmware points the
 * outbound window at a PCI address that appears in no "ranges" entry, and
 * the debug console lives behind it.
 */
