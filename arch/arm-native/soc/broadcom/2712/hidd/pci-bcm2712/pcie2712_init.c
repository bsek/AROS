 ...
 * know of, so it is not listed.
 *
 * The x4 link is adopted rather than trained: the firmware set it up, RP1
 * sits behind it, and the debug console is RP1's UART0. Resetting it would
 * take the console away in the same instruction, with nothing left to
 * report the failure through. PCIE_BridgeSetup() decides that by looking at
 * the link, not from this table - the entry order carries no meaning.
 */
 ...
 * remap register holding the CPU address the window lands on.
 *
 * The size field is log2 less 15 for the large windows, and a separate
 * encoding below 64KB. That small branch is not a curiosity: the 4KB
 * dma-range is the MSI doorbell, and programming it is what lets a device
 * signal at all.
 */
 ...
 * register beside it, with its own enable bit.
 *
 * Only BAR1 through BAR3 are read: they are a contiguous group, and they are
 * the ones this driver programs. A firmware that used one of the higher BARs
 * keeps it, we simply do not describe it - which is harmless as long as
 * nothing asks CPUtoPCI to translate through it.
 */
...
/*
 * This bridge's MSI block, found without following msi-parent: the block's
 * registers sit at the CPU address of the bridge's own 4KB dma-range - the
 * doorbell window SetupInbound() has already programmed - so the mip nodes
 * can be walked by compatible and matched on their first reg entry. That
 * avoids resolving a phandle, which the openfirmware resource offers no
 * call for.
 *
 * msi-ranges is <phandle, type, base, flags, count>. Only an SPI range
 * (type 0) that is edge triggered (flags 1) is one we know how to use, and
 * brcm,msi-offset names the first message the block owns.
 */
/*
 * Depth first for the mip node whose registers are at base.
 *
 * OF_FindNodeByCompatible() cannot do this: its start argument is a subtree
 * to search, not a cursor to resume from, and of_search_compatible() tests
 * that subtree's own root first - so handing a match back returns it again.
 * There are two mip blocks and only one is ours.
 */
 ...
 * programmed it. This runs for a bridge we just trained as well as for one
 * the firmware left running, so everything downstream sees the same thing:
 * what the hardware will actually do, rather than what a property says it
 * ought to.
 *
 * That distinction is not academic. On the x4 link the firmware points the
 * window at a PCI address that appears in no "ranges" entry, and the debug
 * console lives behind it.
 */
 ...
 * firmware has done this before any driver runs, and nothing in AROS will -
 * aHidd_PCIDevice_isMaster covers only bus mastering. Without it the first
 * read of the BAR is an unsupported request, reported as an SError.
 *
 * Bus mastering is left off; the controller cannot fetch a queue entry
 * before its driver has built one, and that driver enables it itself.
 */
