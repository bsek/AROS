 ...
 * on any board we know of. pcie1 is the x1 link - the FPC connector on a Pi 5,
 * the internal M.2 slot on a Pi 500+ - and is what "dtparam=nvme" (alias
 * "pciex1") enables; without that dtparam its node is disabled and there is
 * nothing to drive. pcie2 is the x4 link, hardwired to the RP1 southbridge.
 *
 * One driver object per usable bridge, so the register block size is the only
 * thing they share.
 */
