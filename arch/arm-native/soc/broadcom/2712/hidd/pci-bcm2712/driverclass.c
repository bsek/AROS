...
/*
 * Which bridge this object drives comes in as aHidd_DriverData; the work of
 * getting it running is PCIE_BridgeSetup(), which either trains it or adopts
 * what the firmware left. A bridge that will not come up leaves no object
 * behind, so nothing downstream has to test for one.
 */
...
ULONG PCIE_ReadConfig(struct PCIBcm2712Data *data, UBYTE bus, UBYTE dev, UBYTE sub, UWORD reg)
{
    ...
    else if (dev == 0 && data->link_up)
    {
        ...
        /*
         * The endpoint powered up with whatever its vendor left in the
         * interrupt line field; nothing on this bus has ever written a
         * meaningful value there. Report where the bridge actually raises
         * INTx instead, so a driver that hooks the line it is told about
         * gets the interrupt.
         */
...
/*
 * A bridge we adopted came with its endpoint already placed, and that
 * placement is not ours to move: on the x4 link the firmware put RP1's
 * peripheral window where the debug console now lives behind it.
 *
 * pci.hidd sizes a base register the standard way, by writing all ones and
 * reading back what sticks before restoring it. For however long that takes
 * the endpoint stops decoding its window - and the console is in it, so a
 * bug() from anywhere, including an interrupt, lands on an address nothing
 * claims and the interconnect reports an SError. The four accesses are not
 * even made under Disable().
 *
 * So on an adopted bridge the base registers and the expansion ROM base are
 * read only. The addresses still read correctly, which is what a driver
 * needs to reach its registers.
 *
 * The size does not: sizePCIBaseReg() derives it from what it reads back,
 * and with the write refused that is the address rather than the mask, so
 * aHidd_PCIDevice_SizeN is meaningless for such a device. Nothing consults
 * it today. Reporting a true size would mean emulating the sizing handshake
 * against a value from elsewhere, which is worth doing only once something
 * asks.
 */
